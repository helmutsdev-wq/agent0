...
export function SessionSidebar({ sessions, activeSessionId, onCreate, onSwitch, onDelete, open, onToggle }: SessionSidebarProps) {
  const { t } = useLanguage()

  return (
    <div className={`flex flex-col h-full border-r border-[var(--border)] bg-[var(--bg-secondary)] transition-all duration-200 overflow-hidden ${
      open ? 'w-56' : 'w-0'
    }`}>
      <div className="flex items-center justify-between px-3 py-3 border-b border-[var(--border)] min-w-56">
        <span className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">
          {open && t('session.chats')}
        </span>
        {open && (
          <button
            onClick={onCreate}
      ...
      {open && (
        <div className="flex-1 overflow-y-auto py-1 min-w-56">
          {sessions.length === 0 && (
